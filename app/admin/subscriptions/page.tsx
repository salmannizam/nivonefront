  ...
  const loadSubscriptions = async () => {
    try {
      ...
      if (filter === 'due') {
        ...
        response = await api.get('/admin/plans/subscriptions/all?status=trial');
      } else if (filter === 'paid') {
        response = await api.get('/admin/plans/subscriptions/all');
      } else {
        response = await api.get('/admin/plans/subscriptions/all');
      }
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-fadeIn">
                    ...
                    >
                      <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                        ...
                        <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-1">
                          <code className="bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-lg font-mono">{sub.tenantId.slug}</code>
                        </div>
                      </td>
                      <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                        <div className="text-gray-900 dark:text-white font-bold">{sub.planId.name}</div>
                        ...
                        <span
                          className={`px-3 py-1.5 rounded-full text-xs font-bold ${
                            sub.isPaid
                              ...
                              : 'bg-gradient-to-r from-red-400 to-rose-500 text-white shadow-md animate-pulse-slow'
                          }`}
                        >
                      ...
                      <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm">
                        {!sub.isPaid && (
                          <button
                            ...
                            className="px-4 py-2 text-xs bg-gradient-to-r from-green-600 via-emerald-600 to-teal-600 dark:from-green-500 dark:via-emerald-500 dark:to-teal-500 text-white rounded-xl hover:from-green-700 hover:via-emerald-700 hover:to-teal-700 dark:hover:from-green-600 dark:hover:via-emerald-600 dark:hover:to-teal-600 transition-all shadow-lg hover:shadow-xl font-bold transform hover:scale-105"
                          >
                            Mark Paid
                          </button>
                        )}
      ...
      <div className="lg:hidden space-y-3 sm:space-y-4">
        {subscriptions.length === 0 ? (
          <div className="bg-gradient-to-br from-white via-gray-50 to-gray-100 dark:from-gray-800 dark:via-gray-700 dark:to-gray-600 rounded-2xl shadow-xl border-2 border-gray-200 dark:border-gray-700 p-6 text-center text-gray-500 dark:text-gray-400 text-lg font-bold">
            No subscriptions found
          </div>
        ) : (
          subscriptions.map((sub, index) => {
            ...
            return (
              <div
                key={sub._id}
                className="bg-gradient-to-br from-white via-blue-50/30 to-indigo-50/30 dark:from-gray-800 dark:via-blue-900/20 dark:to-indigo-900/20 rounded-2xl shadow-xl border-2 border-blue-100 dark:border-blue-900/30 p-4 sm:p-5 hover:shadow-2xl transition-all transform hover:scale-[1.02] animate-slideInUp"
                style={{ animationDelay: `${index * 50}ms` }}
              >
